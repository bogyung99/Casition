...
import kr.ac.jbnu.rpicam.enums.ImageEffect;
import kr.ac.jbnu.rpicam.enums.MeteringMode;
 ...
 * the resulting image's encoding type (jpg, bmp, png, gif).
 * <p>
 ...
 * 	RPiCamera camera = new RPiCamera(saveDir).setWidth(150) // Set width property of RPiCamera.
...
public class RPiCamera {
	...
	/**
	 * Creates new RPiCamera. The resulting RPiCamera's save directory will be set
	 * to "/home/pi/Pictures" and will have a default image width and height of 500.
	 * 
	...
	/**
	 * Creates new RPiCamera and sets its save directory. The resulting RPiCamera
	 * will have a default image width and height of 500.
	 * 
	...
	public RPiCamera(String saveDir) throws FailedToRunRaspistillException {
		...
		try {
			...
			// indicate that.
			throw new FailedToRunRaspistillException("RPiCamera failed to run raspistill. The JRPiCam library relies on"
					+ "raspistill to function. Please ensure it is installed and configured" + "on your system.");
		}
	 ...
	 * 	RPiCamera piCamera = new RPiCamera().setEncoding(Encoding.PNG); // Change encoding from JPEG to PNG
	 *
	 * 	// Take a 500x500 PNG image and save it as
	 * 	// "/home/pi/Pictures/AStillImage.png"
	...
	public File takeStill(String pictureName) throws IOException, InterruptedException {
		return takeStill(pictureName, Integer.parseInt(options.get("width")[1]),
				Integer.parseInt(options.get("height")[1]));
	}

	/**
	 * Takes an image of the specified width and height and stores it in a
	 * BufferedImage object. The resulting image is NOT saved anywhere in the Pi's
	 * memory. The image's encoding will be the same as the RPiCamera's encoding
	 * setting (JPEG by default).
	 * <p>
	 * Usage Example:
	 * 
	...
	/**
	 * Takes an image and stores it in a BufferedImage object. The resulting image
	 * is NOT saved anywhere in the Pi's memory. The image's encoding will be the
	 * same as the RPiCamera's encoding setting (JPEG by default).
	 * <p>
	 * Usage Example:
	 * 
	...
	public BufferedImage takeBufferedStill() throws IOException, InterruptedException {
		return takeBufferedStill(Integer.parseInt(options.get("width")[1]), Integer.parseInt(options.get("height")[1]));
	}

	/**
	 * Captures an image and returns the RGB values of that image. Images taken in
	 * this manner are not encoded, rather rather their RGB values are stored in an
	 * int array. Indexes 0 - 2 of the array make up the red, green, and blue values
	 * of the upper left pixel in the image. The pixels are added to the array from
	 * left to right, top to bottom.
	 * <p>
	 * If an image's dimensions are not multiples of 16, then black padding will be
	 * added onto the right and bottom sides of an image until both the width and
	 * height are multiples of 16 (e.g. a 500x500 image would have a black band 12
	 * pixels thick added to the right and bottom sides to achieve dimensions of
	 * 512x512, the closest multiple of 16 to 500).
	 * <p>
	 * The boolean keepPadding parameter may be set to false to remove this padding.
	 * Note that this does require extra computations to be performed, so it has the
	 * potential to slightly reduce the fps a Pi is capable of. However, this has
	 * not yet been officially tested for. If the padding is kept, then it is the
	 * responsibility of the caller to determine what the dimensions of the
	 * resulting image will be, and adjust their processing as such (e.g. If the
	 * padding is kept on a 500x500 image, the array that is returned will contain
	 * 786,432 elements, as opposed to the 750,000 one would expect from a 500x500
	 * image. This is because the image stored in the returned array is not 500x500,
	 * but 512x512 and the caller of this method must take that into account when
	 ...
	 * 	RPiCamera piCamera = new RPiCamera().turnOffPreview().setTimeout(1);
	 *
	 ...
	 * </pre>
	 * 
	 * @param width       An int specifying the width of the image to take (ideally
	 *                    a multiple of 16).
	...
	public int[] takeStillAsRGB(boolean keepPadding) throws IOException {
		return takeStillAsRGB(Integer.parseInt(options.get("width")[1]), Integer.parseInt(options.get("height")[1]),
				keepPadding);
	}

	/**
	 * Take a series of timelapsed photos for the specified time frame and save them
	 * under the specified filename to the RPiCamera's save directory. Length of
	 * time to timelapse for may be specified with the {@link #setTimeout(int)}
	 * method.
	 * <p>
	 * THIS METHOD WILL BLOCK THE THREAD UNTIL THE TIMELAPSE IS COMPLETE IF boolean
	 * wait ARGUMENT IS SET TO true.
	 * <p>
	 * Most recent image in the series may be appended to the file specified in the
	 * {@link #setLinkLatestImage(boolean, String)} method. If this property is set,
	 * timelapse() will return a File object representing the full path to the file
	 * the timelapsed photos are being linked to. Otherwise, will return null.
	 * <p>
	 * When passing in a String for the photo's name, it is important to include
	 * "%04d" in the name. This is where the frame count number will be included in
	 * the file name. If the name does not contain these characters, the frame count
	 * will be appended to the beginning of each file name.
	 * <p>
	 * Usage Example:
	 * 
	 * <pre>
	 ...
	 * 			.setLinkLatestImage(true, "/home/pi/Pictures/linkfile.png"); // Set RPiCamera to link images to
	 * 																			// "linkFile.png"
	 * 	// Begin timelapse
	 * 	piCamera.timelapse(true, // Block thread until timelapse is completed
	 ...
	 *                    image.
	 * @return If the link latest image property is set, the File images are being
	 *         linked to, otherwise returns null.
	 * @throws IOException
	 * @throws InterruptedException
	 */
	...
	// e.printStackTrace();
	// }
	// // If this point is reached, IOException was thrown and failed to retrieve
	// settings,
	// // so return null.
	// return null;
	// }
	 ...
	 * frames per second in this mode is 15fps and the preview will have the same
	 * field of view as the capture. Captures should happen more quickly as no mode
	 * change should be required. This feature is currently under development.
	 */
	...
	/**
	 * Defines the size and location on the screen that the preview window will be
	 * placed. Note the preview will be superimposed over the top of any other
	 * windows/graphics.
	 * 
	 ...
	 * lower than -100 will set contrast to -100, values greater than 100 will set
	 * contrast to 100.
	 * 
	...
	/**
	 * Sets brightness of image (0 to 100), default value is 50. 0 is black, 100 is
	 * white. Brightness values lower than 0 will set brightness to 0, values
	 ...
	 * values lower than -100 will set saturation to -100, values greater than 100
	 * will set saturation to 100.
	 * 
	...
	/**
	 * WARNING: OPERATION NOT YET SUPPORTED BY raspistill SOFTWARE. OPTION MAY STILL
	 * BE SET, BUT WILL HAVE NO EFFECT ON THE IMAGE UNTIL SUPPORT IS ADDED TO
	 * raspistill.<br>
	 * <br>
	...
	/**
	 * Sets Automatic White Balance (AWB) mode. Certain modes may not be supported
	 * by raspistill software, depending on camera type.
	 * 
	...
	/**
	 * Sets an effect to be applied to image. Certain settings may not be supported
	 * by raspistill software, depending on circumstances.
	 * 
	 ...
	 * are applied to the U and Y channels of the image. For example,
	 ...
	 * take any value from 0 upwards, but due to hardware constraints only 0, 90,
	 ...
	 * <p>
	 * So to set a ROI at half way across and down the sensor, and a width and
	 * height of a quarter of the sensor use:
	 * {@code setRegionOfInterest(0.5, 0.5, 0.25, 0.25)}
	 * 
	 * @param x
	 * @param y
	 * @param w
	 ...
	 * increasing the range of dark areas of the image, and decreasing the brighter
	 * areas. This can improve the image in low light areas.
	 * 
	...
	// TODO: MAKE METHOD FOR PREVIEW PARAMETER COMMANDS (Bottom of raspistill -?
	// output)
	 ...
	 * overriden by the {@link #takeStill(String, int, int)} and
	 * {@link #takeBufferedStill(int, int)} methods.
	 * 
	...
	/**
	 * Sets height of images taken by the RPiCamera. Note that this settings can be
	 * overriden by the {@link #takeStill(String, int, int)} and
	 * {@link #takeBufferedStill(int, int)} methods.
	 *
	 ...
	 * Quality values lower than 0 will set quality to 0, values greater than 100
	 * will set quality to 100.
	 * 
	 ...
	 * photography. Allows the latest image in a series of time lapse photos to be
	 * easily accessed while the lapse is still running at the specified file, while
	 * still saving all photos with their unique file names.
	 * 
	 ...
	 * value is 0, then RPiCamera will continue to run indefinitely (but can be
	 * stopped by calling stop() on the instance).
	 * 
	...
	/**
	 * Allows specification of the thumbnail image inserted in to the image file. If
	 ...
	 * types (gif, png, bmp) will take much longer to save than JPG which is
	 * hardware accelerated. Also note that the filename suffix is completely
	...
	// TODO: ADD EXIF DATA OPTION HERE, NEED TO TEST ON RPI TERMINAL TO KNOW HOW TO
	// IMPLEMENT
	...
	/**
	 * Turns on burst mode for RPiCamera. Prevents raspistill from switching between
	 * preview and capture modes, saving a few 100ms of time for each capture.
	 */
	...
	/**
	 * Replaces output pattern (%d) in save file name with DateTime
	 * (MonthDayHourMinSec). E.g.
	 * 
	 * <pre>
	 * {
	 * 	&#64;code
	 * 	RPiCamera camera = new RPiCamera().setDateTimeOn();
	 * 	camera.takeStill("APicture%d.jpg", 500, 500);
	 * }
	 * </pre>
	 * 
	 * Would produce a file named "APicture1223140756" if executed on Dec 23rd, at
	 * 14:07:56.
	 */
	...
	/**
	 * Disables effects of {@link #setDateTimeOn()}. This is default state of
	 * {@link RPiCamera}.
	 */
	 ...
	 * {
	 * 	&#64;code
	 * 	RPiCamera camera = new RPiCamera().setTimestampOn();
	 * 	camera.takeStill("APicture%d.jpg", 500, 500);
	 * }
	 * </pre>
	 * 
	 * Would produce a file named "APictureXXX", where XXX is the number of seconds
	 * since 1970.
	 */
	...
	/**
	 * Disables effects of {@link #setTimestampOn()}. This is default state of
	 * {@link RPiCamera}.
	 */
